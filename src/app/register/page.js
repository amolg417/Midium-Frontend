...
const Register = () => {
  ...
  function Validation(Name, Email, avatar,Password) {
    ...
    if (Name.trim() === "" || Email.trim() === "" || !avatar?.name || Password.trim()==="") {
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className={styles.form}>
     <label htmlFor='profile'><div className={styles.circle}>Profile Pic</div></label>
     <input type='file' id='profile' name='avatar' style={{display:"none"}} onChange={(e)=>setAvatar(e.currentTarget.files[0])}/> 
      <input
        type="text"
